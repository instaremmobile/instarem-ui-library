...
const Button = React.forwardRef(({ children, variant = "primary", size = "medium", isLoading = false, disabled = false, fullWidth = false, className = "", startIcon, endIcon, type = "button", ...rest }, ref) => {
    ...
    return (jsxs("button", { ref: ref, type: type, className: combinedClassName, disabled: disabled || isLoading, "aria-disabled": disabled || isLoading, ...rest, children: [isLoading && (jsx("span", { className: "btn__spinner", "aria-hidden": "true", children: jsx("svg", { viewBox: "0 0 24 24", className: "btn__spinner-icon", children: jsx("circle", { cx: "12", cy: "12", r: "10", stroke: "currentColor", strokeWidth: "2", fill: "none", strokeLinecap: "round", strokeDasharray: "31.416", strokeDashoffset: "31.416" }) }) })), startIcon && !isLoading && (jsx("span", { className: "btn__icon btn__icon--start", "aria-hidden": "true", children: startIcon })), jsx("span", { className: `btn__text ${isLoading ? "btn__text--loading" : ""}`, children: children }), endIcon && !isLoading && (jsx("span", { className: "btn__icon btn__icon--end", "aria-hidden": "true", children: endIcon }))] }));
});
...
const ToggleComponent = forwardRef(({ id, className, onChange, labelPosition = "right", checked, label, name, ...props }, ref) => {
    const toggleId = id || useId();
    return (jsxs("label", { htmlFor: toggleId, className: "toggle-switch__container", children: [label && labelPosition === "left" ? (jsx("span", { className: "toggle-switch__label", children: label })) : null, jsxs("div", { className: "toggle-switch__container", children: [jsx("input", { ...props, id: toggleId, ref: ref, type: "checkbox", className: cn("toggle-switch__input", className), checked: checked, name: name, onChange: (event) => onChange?.(event.target.checked) }), jsx("div", { className: "toggle-switch__track", children: jsx("div", { className: "toggle-switch__thumb" }) })] }), label && labelPosition === "right" ? (jsx("span", { className: "toggle-switch__label", children: label })) : null] }));
});
...
const InputField = forwardRef(({ className = '', helperText, type = 'text', label, error, shrink, value: controlledValue, startAdornment, endAdornment, disabled, id, defaultValue = '', onIconClick, iconSize = 18, clearable, fullWidth = false, suggestions = [], isSearchable = false, onBlur, onFocus, fetchFunction, retryConfig = { maxAttempt: 5 }, handleChange, outlined = false, borderless = false, format, parse, formatOn = 'blur', rawOnChange = true, maxRawLength, ...props }, ref) => {
    const [isFocused, setIsFocused] = useState(false);
    const [internalValue, setInternalValue] = useState(defaultValue);
    const [selectedSuggestionIndex, setSelectedSuggestionIndex] = useState(-1);
    const [filteredSuggestions, setFilteredSuggestions] = useState([]);
    const [originalFetchedSuggestions, setOriginalFetchedSuggestions] = useState([]);
    ...
    const normalizeSuggestions = useCallback((arr) => {
        if (!arr || arr.length === 0)
            return [];
        return arr.map((item) => {
            ...
            if (newValue.trim()) {
                ...
                if (searchResults.length > 0) {
                    searchResults.forEach((resultLabel) => {
                        const hit = allSuggestions.find((item) => item.label === resultLabel ||
                ...
                if (matched.length === 0) {
                    ...
                    setFilteredSuggestions(allSuggestions.filter((i) => i.label.toLowerCase().includes(q) ||
        ...
        switch (event.key) {
            case 'ArrowUp': {
                ...
                setSelectedSuggestionIndex((prev) => {
                    ...
                    setTimeout(() => {
                        const el = suggestionListRef.current?.children[ni];
    ...
    const handleInputChange = useCallback((event) => {
        ...
        if (isSearchable) {
            ...
            if (!isControlled)
                setInternalValue(nextDisplay);
            return;
        }
        setDisplayText(nextDisplay);
        const raw = parseSafe(nextDisplay);
        if (typeof maxRawLength === 'number' &&
    ...
    const renderSuggestions = useCallback((suggestion, query) => {
        ...
        return (jsx("span", { children: parts.map((part, index) => index % 2 === 1 ? (jsx("span", { className: "highlight", children: part }, index)) : (jsx("span", { children: part }, index))) }));
    }, []);
    useEffect(() => {
...
const Checkbox = forwardRef(({ id, label, checked = false, disabled = false, indeterminate = false, onChange, className = "", "aria-describedby": ariaDescribedBy, "aria-labelledby": ariaLabelledBy, required = false, }, ref) => {
    const inputRef = useRef(null);
    ...
    return (jsxs("label", { className: containerClasses, htmlFor: id, children: [jsx("input", { ref: inputRef, type: "checkbox", id: id, className: "checkbox-input", checked: checked, disabled: disabled, onChange: handleChange, "aria-describedby": ariaDescribedBy, "aria-labelledby": ariaLabelledBy, required: required }), jsx("span", { className: "checkbox-custom", "aria-hidden": "true" }), jsx("span", { className: labelClasses, children: label })] }));
});
...
const RadioButton = React.forwardRef(({ name, value, checked = false, disabled = false, label, className = "", size = "medium", variant = "primary", onChange, onFocus, onBlur, "aria-label": ariaLabel, "aria-describedby": ariaDescribedBy, ...props }, ref) => {
    ...
    return (jsxs("div", { className: cn("radio-button", `radio-button--${size}`, `radio-button--${variant}`, disabled ? "radio-button--disabled" : "", checked ? "radio-button--checked" : "", className), children: [jsx("input", { ref: ref, id: inputId, type: "radio", name: name, value: value, checked: checked, disabled: disabled, onChange: handleChange, onKeyDown: handleKeyDown, onFocus: onFocus, onBlur: onBlur, className: "radio-button__input", "aria-label": ariaLabel, "aria-describedby": ariaDescribedBy, ...props }), jsxs("label", { htmlFor: inputId, className: "radio-button__label", children: [jsx("span", { className: "radio-button__control", children: jsx("span", { className: "radio-button__indicator" }) }), label && jsx("span", { className: "radio-button__text", children: label })] })] }));
});
RadioButton.displayName = "RadioButton";

export { Button, Checkbox, Input, RadioButton, Toggle };
