...
const InputField = React.forwardRef(({ className = '', helperText, type = 'text', label, error, shrink, value: controlledValue, startAdornment, endAdornment, disabled, id, defaultValue = '', onIconClick, iconSize = 18, clearable, fullWidth = false, suggestions = [], isSearchable = false, onBlur, onFocus, fetchFunction, retryConfig = { maxAttempt: 5 }, handleChange, outlined = false, borderless = false, format, parse, formatOn = 'blur', rawOnChange = true, maxRawLength, ...props }, ref) => {
    ...
    const [displayText, setDisplayText] = React.useState(String(defaultValue ?? ''));
    ...
    const formatSafe = React.useCallback((v) => (format ? format(v) : v ?? ''), [format]);
    const parseSafe = React.useCallback((s) => (parse ? parse(s) : s), [parse]);
    ...
    const normalizeSuggestions = React.useCallback((arr) => {
        if (!arr || arr.length === 0)
            return [];
        return arr.map((item) => {
    ...
    const renderSuggestions = React.useCallback((suggestion, query) => {
        ...
        return (jsxRuntime.jsx("span", { children: parts.map((part, index) => index % 2 === 1 ? (jsxRuntime.jsx("span", { className: "highlight", children: part }, index)) : (jsxRuntime.jsx("span", { children: part }, index))) }));
    }, []);
    React.useEffect(() => {
